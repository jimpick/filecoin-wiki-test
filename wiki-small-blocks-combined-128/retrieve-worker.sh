	...
	for d in $(echo $DEALS | shuf); do
      ...
      if [ "$?" != "0" ]; then
        ...
        continue
			...
			/usr/bin/time timeout -k 23m 22m lotus client retrieve --miner=$MINER --maxPrice=0.000000005000000000 $CID $PWD/$TARGET_DIR/$x-$MINER-$DEAL-$TIMESTAMP.bin 2>&1 | tee -a $TARGET_DIR/$x-$MINER-$DEAL-$TIMESTAMP.log
