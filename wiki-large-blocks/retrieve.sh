...
for x in *.zip.??; do
	...
	for d in $DEALS; do
		...
		/usr/bin/time timeout -k 11m 10m lotus client retrieve --miner=$MINER $CID $PWD/retrievals/$x-$MINER-$DEAL-$TIMESTAMP.bin 2>&1 | tee -a $PWD/retrievals/$x-$MINER-$DEAL-$TIMESTAMP.log
		sleep 5
	done
	echo
	sleep 1
